...
Implementa EXATAMENTE o BLOCKER-DESIGN-v0.1.json, ja congelado por hash."""
import json, re, hashlib, pathlib, unicodedata, collections, itertools

D = json.loads(pathlib.Path("BLOCKER-DESIGN-v0.1.json").read_text(encoding="utf-8"))
...
json.dump(F, open("features.json", "w"), ensure_ascii=False, indent=1)
...
json.dump(MAP, open("control-mappings.json", "w"), ensure_ascii=False, indent=1)
...
json.dump({"metrics": MET, "satisfying": ok, "chosen": chosen,
          ...
          open("variant-metrics.json", "w"), ensure_ascii=False, indent=1)
json.dump([{k: v for k, v in p.items() if not k.startswith("_")} for p in PAIRS],
          open("pair-traces.json", "w"), ensure_ascii=False, indent=1)
