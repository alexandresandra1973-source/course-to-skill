"""FUSION R4 — consumo real de candidates admitidos e transporte verificavel.
...
CONSUMPTION_VERSION = "r4-candidate-consumption-v1"
TRANSPORT_VERSION = "r4-workflow-transport-v1"
...
def materialize(cand_by_kind, admission, sph):
    ...
    for kind, pop in POP.items():
        for c in cand_by_kind.get(kind, []):
            ...
            st = STRUCT[kind](c)
            out[pop].append({
                "fusion_local_id": f"F-{pop.upper()[:2]}-{lid}",
                "candidate_ref": {"source_package_hash": sph, "local_id": lid},
                "kind": kind,
                "structure": st,
...
def fusion_id(source_package_hashes, fusion_config_hash, admission_report_hash,
              admitted_set_hash, outputs_hash):
