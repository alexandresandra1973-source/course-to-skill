"""CANDIDATE ADMISSION POLICY v0.1 — implementacao mecanica, offline, zero modelo.
...
def admit_one(kind, c, ev_ids, claim_ids, seen_ids):
    ...
    lid = c.get("local_id")
    if not isinstance(lid, str) or not lid.strip() or lid in seen_ids:
    ...
    for kind in ("rule_candidates", "workflow_candidates", "anti_pattern_candidates"):
        for c in cand.get(kind, []):
            ...
            if isinstance(c.get("local_id"), str):
                seen.add(c["local_id"])
            r["qualified_ref"] = [source_package_hash, r["local_id"]]
